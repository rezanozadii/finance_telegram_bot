...
export function Friends() {
  ...
  const formatBalances = (balances: Record<string, number>): { text: string; color: string } => {
    const entries = Object.entries(balances).filter(([, v]) => Math.abs(v) >= 0.01);
    if (entries.length === 0) return { text: t('settled_label'), color: '#34c759' };

    const parts = entries.map(([currency, amount]) => {
      const abs = Math.abs(amount).toFixed(2);
      if (amount > 0) return `${t('they_owe')} ${currency} ${abs}`;
      return `${t('you_owe')} ${currency} ${abs}`;
    });
  ...
  return (
    <List>
      {friends.length === 0 && (
        <Section>
          <Cell subtitle={t('add_friend_hint')}>{t('no_friends')}</Cell>
        </Section>
      )}

      {withBalance.length > 0 && (
        <Section header={t('open_balances')}>
          {withBalance.map((f) => {
